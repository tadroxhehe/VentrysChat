    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("ec")
                ...
                .executes(ctx -> {
                    ...
                    if (EcSavedData.get(player.getLevel()).panelCount() == 0) {
                        player.sendMessage(new TextComponent("§eAucun panneau EC. Créez-en un avec §f/ec create <nom>§e."), player.getUUID());
                    }
                    return 1;
                })
                        ...
                        .then(Commands.argument("nom", StringArgumentType.word())
                                .executes(ctx -> {
                                    ...
                                    if (!VALID_NAME.matcher(name).matches()) {
                                        player.sendMessage(new TextComponent("§cNom invalide (1-32 caractères : lettres, chiffres, _ ou -)."), player.getUUID());
                                        return 0;
                                    }
                                    EcSavedData data = EcSavedData.get(player.getLevel());
                                    if (data.panelCount() >= EcSavedData.MAX_PANELS) {
                                        player.sendMessage(new TextComponent("§cLimite de panneaux EC atteinte (" + EcSavedData.MAX_PANELS + ")."), player.getUUID());
                                        return 0;
                                    }
                                    if (!data.createPanel(name)) {
                                        player.sendMessage(new TextComponent("§cUn panneau EC nommé §f" + name + " §cexiste déjà."), player.getUUID());
                                        return 0;
                                    }
                                    player.sendMessage(new TextComponent("§aPanneau EC « §f" + name.trim() + "§a » créé avec 8 coffres."), player.getUUID());
                                    return 1;
                                }))));
    }
}
